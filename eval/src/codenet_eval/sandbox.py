...
import io
import glob
from eval_instruction_tool import exec_with_timeout
...
def run_python_code_on_inputs(
    ...
    """
    Run the given code on the inputs for the given problem_id, and returns (avg_instructions, std_instructions, avg_acc).
    ...
    with open(code_path,'r') as f:
        code = f.read()
    million_instruction, accs = [], []
    per_trial_instructions = []
    for test_case_idx in range(num_test_cases):
        # if additional_info=='p00447':
        #     print('stop')
        input_file_path = f"{unit_test_data_basepath}/input.{test_case_idx}.txt"
        # logging.info(f"Running command: {cmd} < {input_file_path}")
        _per_trial_million_instructions = []
        timeout_times=0
        for trial_idx in range(num_runs_per_test_case):
            try:
                err, output, instruction_taken = exec_with_timeout(code,input_file_path,max_seconds_per_run)
                if err=='TimeoutError':
                    raise TimeoutError
                # count instructions in million
                instruction_taken=int(instruction_taken)/1000000
                _per_trial_million_instructions.append(instruction_taken)
                if output is None:
                    return (np.nan, np.nan, 0)
                    # timeout: since we have a generous timeout, this should not happen

                if trial_idx >= ignore_first_k:
                    million_instruction.append(instruction_taken)
                    if ground_truths is not None:
                        accuracy = get_accuracy(output, ground_truths[test_case_idx])
                        # if accuracy != 1:
                        #     print(f'heres a mistake,{additional_info}.{test_case_idx}.{trial_idx}')
                        if return_if_acc_below is not None and accuracy < return_if_acc_below:
                            ...
                            return (instruction_taken, 0, accuracy)
                        accs.append(accuracy)
            except TimeoutError:
                return (np.nan, np.nan, 0)
                # timeout_times+=1 
                # # print(f"Timeout for {additional_info}.{test_case_idx}.{trial_idx}")
                # if timeout_times>stand_k_timeout:
                #     return (np.nan, np.nan, 0)
                # else:
                #     continue
        ...
        per_trial_instructions.append(_per_trial_million_instructions)

    million_instruction, accs = np.array(million_instruction), np.array(accs)
    if return_per_trial_times and ground_truths is None:
        return per_trial_instructions  # type: ignore
    if return_dict:
        return {
            "avg_instructions": np.mean(million_instruction),
            "std_instructions": np.std(million_instruction),
            "avg_acc": np.mean(accs),
        }
    else:
        return np.mean(million_instruction), np.std(million_instruction), np.mean(accs)  # type: ignore
