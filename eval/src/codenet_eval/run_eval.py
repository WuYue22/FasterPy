...
def evaluate_generated_outputs(cfg: EvaluationConfig) -> None:
    ...
    print(f"Number of programs to evaluate: {len(merged)}")
    print(f"Number of trials per program: {cfg.num_trials}")
    print(f"Number of trials to ignore: {cfg.ignore_first_k}")
    print(f"Maximum time per run: {cfg.max_time_per_run}")
    print(f"Input column: {cfg.slow_code_col}")
    print(f"Reference column: {cfg.reference_code_col}")
    print(f"Model generated column: {cfg.model_generated_potentially_faster_code_col}")
    print(f"inputs/outputs basepath: {cfg.inputs_outputs_basepath}")
...
def read_inputs_and_prepare(cfg) -> pd.DataFrame:
    ...
    print(f"Reading reference file from {cfg.reference_file_path}")
    print(f"Reading model generated outputs from {cfg.model_generated_outputs_path}")
    ...
    print(f"Read {len(gen_df)} rows from {cfg.model_generated_outputs_path}")
...
def _prepare_for_rerun(df: pd.DataFrame, cfg: EvaluationConfig) -> pd.DataFrame:
    ...
    print("ALERT! THIS IS A RERUN")
    print("Preparing for rerun...")
    print(f"Found accuracy column: {acc_column}, {len(df)} rows")
...
def write_programs_read_ground_truth(
    ...
    print(f"finished writing programs to {output_code_location}")
...
def run_programs(
    ...
    print(f"Ran for {len(results)} problems")
    return results
...
def print_summary(cfg, merged, results, gen_col: str):
    ...
    print(f"Writing report to {cfg.output_report_file_path} with {len(report_rows)} rows")
    ...
    print("-" * 80)
    print(f"[Our measurement] input program (ms): {mean_std(run_metrics, 'input_time')}")
    print(
    ...
    if len(run_metrics_improved) > 0:
        ...
        print(
            f"Found {len(run_metrics_improved)} problems where the {gen_col} program is faster than the reference program"
        )
        print(
